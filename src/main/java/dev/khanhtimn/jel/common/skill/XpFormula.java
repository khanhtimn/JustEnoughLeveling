...
public sealed interface XpFormula
    ...
    static XpFormula constant(int xpPerLevel) {
        return new Constant(xpPerLevel);
    }

    static XpFormula linear(int base, int increment) {
        return new Linear(base, increment);
    }
    ...
    MapCodec<XpFormula> MAP_CODEC = new MapCodec<>() {
        ...
        public <T> DataResult<XpFormula> decode(
            ...
            return Codec.STRING.parse(ops, typeElem).flatMap(type -> switch (type) {
                case "vanilla" ->
                    Vanilla.MAP_CODEC.decode(ops, input).map(v -> v);
                case "constant" ->
                    Constant.MAP_CODEC.decode(ops, input).map(v -> v);
                case "linear" ->
                    Linear.MAP_CODEC.decode(ops, input).map(v -> v);
                case "exponential" ->
                    Exponential.MAP_CODEC.decode(ops, input).map(v -> v);
                default ->
                    DataResult.error(() -> "Unknown XpFormula type: " + type);
            });
        }
        ...
        public <T> RecordBuilder<T> encode(
            ...
            return switch (input) {
                case Vanilla vanilla ->
                    Vanilla.MAP_CODEC.encode(vanilla, ops, builder);
                case Constant constant ->
                    Constant.MAP_CODEC.encode(constant, ops, builder);
                case Linear linear ->
                    Linear.MAP_CODEC.encode(linear, ops, builder);
                case Exponential exponential ->
                    Exponential.MAP_CODEC.encode(exponential, ops, builder);
            };
        }
